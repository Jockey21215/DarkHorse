...
const UpdateBlogPage = () => {
    ...
    const onUpdateBlog = async (
        ...
        try {
            const response = await fetch(`${API_URL}/blogs/update/${id}`, {
                method: "PUT",
                body: JSON.stringify({
                    title,
                    text,
    ...
    return (
        <BlogAddEditComponent data={{ title: blog.title, text: blog.text, imageUrl: blog.imageUrl }} setData={onUpdateBlog}></BlogAddEditComponent>
    );
};
